...
adminRoutes.get("/tax-definitions", async (c) => {
  try {
    const list = await getTaxDefinitions();
    return c.json(list);
  } catch (e) {
    return c.json({ error: String(e) }, 500);
  }
...
adminRoutes.get("/tax-definitions/:id", async (c) => {
  try {
    const id = c.req.param("id");
    const tax = await getTaxDefinitionById(id);
...
adminRoutes.post("/tax-definitions", async (c) => {
  const data = await c.req.json();
  try {
    const created = await createTaxDefinition(data);
...
adminRoutes.put("/tax-definitions/:id", async (c) => {
  ...
  try {
    const id = c.req.param("id");
    const updated = await updateTaxDefinition(id, data);
...
adminRoutes.delete("/tax-definitions/:id", async (c) => {
  try {
    const id = c.req.param("id");
    const result = await deleteTaxDefinition(id);
    return c.json(result);
  } catch (e) {
    const msg = String(e);
    if (msg === "NOT_FOUND") return c.json({ error: "Not found" }, 404);
...
adminRoutes.post("/templates/install-from-manifest", async (c) => {
  try {
    ...
    try {
      ...
      await setSetting(`templateSource:${t.id}`, url);
    } catch (_e) { /* non-fatal */ }
    return c.json(t);
  } catch (e) {
...
adminRoutes.put("/settings", async (c) => {
  ...
  try {
    if ("logoUrl" in data) await deleteSetting("logoUrl");
  ...
  if (typeof data.templateId === "string" && data.templateId) {
    try {
      await setDefaultTemplate(String(data.templateId));
    } catch { /* ignore */ }
  }
...
adminRoutes.get("/products/:id/usage", async (c) => {
  const id = c.req.param("id");
  const product = await getProductById(id);
  if (!product) {
    return c.json({ error: "Product not found" }, 404);
  }
  const usedInInvoices = await isProductUsedInInvoices(id);
  return c.json({ usedInInvoices });
});

// Reactivate a soft-deleted product
adminRoutes.post("/products/:id/reactivate", async (c) => {
  const id = c.req.param("id");
  try {
...
adminRoutes.get("/product-categories/:id", async (c) => {
  const id = c.req.param("id");
...
adminRoutes.get("/product-categories/:id/usage", async (c) => {
  const id = c.req.param("id");
  const usage = await isCategoryUsed(id);
  return c.json(usage);
});

// Product Units
adminRoutes.get("/product-units", async (c) => {
  const units = await getUnits();
  return c.json(units);
});

adminRoutes.get("/product-units/:id", async (c) => {
  const id = c.req.param("id");
...
adminRoutes.delete("/product-units/:id", async (c) => {
  const id = c.req.param("id");
  try {
...
adminRoutes.get("/product-units/:id/usage", async (c) => {
  const id = c.req.param("id");
  const usage = await isUnitUsed(id);
  return c.json(usage);
});
...
adminRoutes.get("/export/full", async (c) => {
  ...
  const dbPath = (typeof Deno !== "undefined" ? Deno.env.get("DATABASE_PATH") : undefined) || "./invio.db";
