...
export async function installTemplateFromManifest(manifestUrl: string) {
  ...
  const saved = await upsertTemplateWithId(manifestId, {
...
export async function upsertTemplateWithId(
  id: string,
  data: Partial<Template>,
): Promise<Template | undefined> {
  const db = getDatabase();
  if (data.isDefault === true) {
    await db.query("UPDATE templates SET is_default = 0 WHERE id != ?", [id]);
  }
  await db.query(
...
export async function setDefaultTemplate(id: string): Promise<boolean> {
  ...
  await db.query("UPDATE templates SET is_default = 1 WHERE id = ?", [id]);
  return true;
}
